...
from nn_pipeline.utils import load_model as load_nn_model
from nn_pipeline.utils import predict as predict_nn
from gpr_pipeline.utils import load_model as load_gpr_model
from gpr_pipeline.utils import predict as predict_gpr
...
@st.cache_resource
def initialize_gpr():
    return load_gpr_model()


nn_model, nn_scalers = initialize_nn()
gpr_model, gpr_scalers = initialize_gpr()
...
st.markdown(
"""
Predict **Bubble Point Pressure (Pb)** from reservoir-fluid composition using
either a **Neural Network (NN)** or **Gaussian Process Regression (GPR)**.
"""
)
...
selected_model = st.radio(
    ...
    ["Neural Network (NN)", "Gaussian Process Regression (GPR)"],
    horizontal=True
)
...
if predict_button:
    ...
    with st.spinner("Predicting Bubble Point Pressure..."):
        ...
        if selected_model == "Neural Network (NN)":
            result = predict_nn(
                nn_model, nn_scalers,
                T, N2, C1, C2, C3, C4,
                C5, C6, C7, MWC7, H2S, CO2,
            )
        else:
            result = predict_gpr(
                gpr_model, gpr_scalers,
                T, N2, C1, C2, C3, C4,
                C5, C6, C7, MWC7, H2S, CO2,
            )
